...
public class LogicController {
    ...
    public String calculate(@RequestParam("start") double start, @RequestParam("end") double end,
                            @RequestParam("step") double step, @RequestParam("a") double a,
                            @RequestParam("b") double b, @RequestParam("c") double c, Model model) {
        if (end < start) {
            throw new IllegalArgumentException();
        }

        xValues = logic.xValuesArrayFill(start, end, step);
        yValues = logic.yValuesArrayFill(xValues, a, b ,c);
